...
try {
  ...
  let html_url
  if (isComment) {
    const title = (`@${author}: ` + noQuoteArray.join('FC4A4130-2507-41B5-9321-82B2A0659C80\n').replaceAll('\n', ' ')).substring(0, 255)
    html_url = payload.comment.html_url
    const new_issue = octokit.issues.create({
      owner: payload.repository.owner.login,
      repo: payload.repository.name,
  ...
  core.setFailed(error.message);
}The protocol response was successful（errr。message）
  Please wait for the protocol and source code to stabilize and then try to merge and generate again
